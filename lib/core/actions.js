...
const createComponentAction = async (componentName, dest) => {
  const ext = program.language === 'javascript' ? 'js' : 'ts'
  ...
  // 渲染文件
  const componentPath = path.resolve(outerDirPath, `${componentName}.component.${ext}x`)
  const indexPath = path.resolve(outerDirPath, `index.${ext}`)
  const stylePath = path.resolve(outerDirPath, `${componentName}.style.${ext}`)
...
const createStore = async (pageName, dest) => {
  ...
  // // 生成文件
  const reducerPath = path.resolve(storePath, `reducer.${ext}`)
  const actionCreatorPath = path.resolve(storePath, `actionCreators.${ext}`)
  const constantsPath = path.resolve(storePath, `constants.${ext}`)
...
const createTypeFile = async (pageName) => {
  ...
  const typeName = `${pageName}.type.${ext}`
...
const createPageAction = async (pageName, dest) => {
  const isPure = program.pure || false
